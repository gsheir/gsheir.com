...
from game.models import Round, League, UserSelection, ProvisionalSelection, LeagueStanding, SelectionOrder, Goal
    ...
    def handle(self, *args, **options):
        ...
        if round_id:
            try:
                ...
                self.stdout.write(self.style.ERROR(f'Round with ID {round_id} does not exist'))
        else:
    ...
    def process_round_selections(self, round_obj):
        ...
        for league in leagues:
            ...
            for selection_order in selection_orders:
                ...
                if UserSelection.objects.filter(user=user, league=league, round=round_obj).exists():
                    continue
                ...
                for provisional in provisional_selections:
                    if provisional.player.id not in selected_players and selected_count < 3:
                        ...
                        self.stdout.write(f'    {user.username} selected {provisional.player.name}')
                
                if selected_count < 3:
                    self.stdout.write(f'    Warning: {user.username} only selected {selected_count} players')
    ...
    def process_round_completion(self, round_obj):
        ...
        for league in leagues:
            ...
            for selection in user_selections:
                ...
                if goals_in_round > 0:
                    self.stdout.write(f'    {selection.user.username}: {selection.player.name} scored {goals_in_round} goals')
            
            # Update or create league standings
            for user, points in user_points.items():
                # Get previous total points
                previous_standing = LeagueStanding.objects.filter(
                    league=league,
                    user=user,
                    round__number__lt=round_obj.number
                ).order_by('-round__number').first()
                
                previous_total = previous_standing.total_points if previous_standing else 0
                ...
                self.stdout.write(f'    {user.username}: {points} points this round, {new_total} total')
        ...
        self.stdout.write(self.style.SUCCESS(f'Round {round_obj.name} processing completed'))
    ...
    def ensure_selection_order(self, league, round_obj):
        ...
        if round_obj.number == 1:
            ...
            if previous_round:
                ...
                ).order_by('total_points', '?')  # ? for random tie-breaking
