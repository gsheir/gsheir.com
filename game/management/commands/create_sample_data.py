...
from game.models import League, LeagueParticipant, Player, Round, Team
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for name, country in team_names:
            team, created = Team.objects.get_or_create(name=name, country=country)
            if created:
        ...
        for i, name in enumerate(player_names):
            ...
            player, created = Player.objects.get_or_create(
                name=name,
                team=team,
                defaults={"position": position, "goals_scored": goals},
            )
            if created:
        ...
        league, created = League.objects.get_or_create(
            name="Sample League",
            defaults={"code": "SAMPLE", "created_by": users[0], "max_participants": 5},
        )
        if created:
