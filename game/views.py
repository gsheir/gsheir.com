...
from .models import (
    League, LeagueParticipant, Round, Player, UserSelection, 
    ProvisionalSelection, LeagueStanding, SelectionOrder, Goal
)
...
class HomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_leagues = LeagueParticipant.objects.filter(user=self.request.user).select_related('league')
...
class CreateLeagueView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'League created successfully! League code: {code}')
        return response
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                if LeagueParticipant.objects.filter(league=league, user=request.user).exists():
...
class LeagueView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not LeagueParticipant.objects.filter(league=league, user=self.request.user).exists():
        ...
        if current_round:
            ...
            selections = UserSelection.objects.filter(
                league=league,
                round=current_round
            ).select_related('user', 'player').order_by('user__username', 'selection_order')
...
class SelectionView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        now = timezone.now()
        selection_open = current_round.selection_opens <= now <= current_round.selection_closes
        context['selection_open'] = selection_open
        
        # Get all players
        players = Player.objects.all().select_related('team').order_by('-goals_scored', 'name')
        ...
        provisional_selections = ProvisionalSelection.objects.filter(
            ...
            league=league,
            round=current_round
        ).select_related('player').order_by('priority')
...
class PlayerListAPIView(LoginRequiredMixin, View):
    def get(self, request):
        players = Player.objects.all().select_related('team').values(
            'id', 'name', 'team__name', 'position', 'goals_scored'
...
class ProvisionalSelectionAPIView(LoginRequiredMixin, View):
    def post(self, request):
        try:
            ...
            # Check if user is in league
            if not LeagueParticipant.objects.filter(league=league, user=request.user).exists():
                return JsonResponse({'error': 'Not authorized'}, status=403)
            
            # Check if selection is still open
            now = timezone.now()
            if not (current_round.selection_opens <= now <= current_round.selection_closes):
    ...
    def get(self, request):
        ...
        provisional_selections = ProvisionalSelection.objects.filter(
            ...
            league_id=league_id,
            round_id=round_id
        ).select_related('player', 'player__team').order_by('priority')
